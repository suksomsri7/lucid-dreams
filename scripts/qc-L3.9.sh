...
chk H5.4 "$(cd $m && rm -rf ios && timeout 300 npx expo prebuild -p ios --no-install >/dev/null 2>&1; r=$?; a=$(npx expo-modules-autolinking search -p ios 2>/dev/null | grep -c "lucid-audio-route"); git -C "$root" checkout -- targets/watch/Assets.xcassets/AppIcon.appiconset/Contents.json 2>/dev/null; rm -f "$root"/targets/watch/Assets.xcassets/AppIcon.appiconset/App-Icon-1024x1024@1x.png; [ $r -eq 0 ] && [ "$a" -ge 1 ] && echo 1 || echo 0)" "prebuild ios ผ่าน + expo-modules-autolinking เห็น lucid-audio-route (Podfile จะมีตอน pod install เท่านั้น — VPS ไม่มี cocoapods)"
