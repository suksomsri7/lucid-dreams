...
| `StreakStore.swift` | (L2.2n) เขียนสตรีคลง App Group `group.app.dreaming` คีย์ `streak.nights` / `streak.updatedAt` + `WidgetCenter.reloadAllTimelines()` → ตัวอ่านคือ `targets/watch-complication` |
...
3. ~~`WKCompanionAppBundleIdentifier` ไม่ตรง~~ **แก้แล้วใน L2.2n**: เดิมเป็น `app.luciddream.ios` ซึ่ง**ไม่ตรง** `ios.bundleIdentifier` จริง (`app.dreaming.ios`) — ถ้าไม่แก้ นาฬิกาจะไม่จับคู่กับแอปเลย · R1 แค่ยืนยันว่าจับคู่ติดจริงบนเครื่อง
