/**
 * "Find another device" — the body of the page behind the `+` row on every devices screen
 * (WO L2.3, replacing the "coming soon" sheet L1.3 left as debt D-4).
 *
 ...
 *
 * ## Why this is a component in `src/devices/` and not a route (WO L3.9 §A)
 *
 * It used to *be* `app/plan/find-devices.tsx`, and onboarding pushed that path. But
 * `app/_layout.tsx` redirects anything outside `/onboarding` back to `/onboarding` until
 * onboarding is finished — so during onboarding, tapping "find another device" bounced the user to
 * the first onboarding screen (the owner's TestFlight 0.1.0 (1) report, bug 1). The fix is a second
 * route *inside* onboarding (`app/onboarding/find-devices.tsx`) rather than a weaker guard, and
 * two routes must not mean two copies of this screen — so the screen lives here and both routes are
 * four-line wrappers that pass `onDone`.
 *
 * It deliberately does **not** require tonight's plan (unlike its `app/plan/` siblings): it runs
 * during onboarding too, before any plan exists.
 */
...
import { applyBleSelectFixture, bleFixtureRequested, fixtureBleScanResults } from '../dev/fixtures';
import { setPhoneOnMattress, useSensorPrefs } from './prefs';
import { refreshDevicesFromPlatform } from './registry';
import { useT, type TranslationKey } from '../i18n';
import { getPlatform } from '../platform';
import type { BleAvailability, BleScanResult, Unsubscribe } from '../platform';
import {
  Button,
  GlassCard,
  Icon,
...
export interface FindDevicesScreenProps {
  /**
   * Where "done"/back goes. Both wrappers pass `router.back()`, which returns to the devices
   * screen the `+` row was tapped on — the onboarding one or the pre-night one — with its state
   * refreshed (every `pick()`/toggle below calls `refreshDevicesFromPlatform()`, and that screen
   * is subscribed to the registry).
   */
  onDone: () => void;
}

export function FindDevicesScreen({ onDone }: FindDevicesScreenProps) {
  const { t } = useT();
